...
bool queue_push_front(struct TQueue* aQueue, TQueueElement aValue)
	...
	if (!aQueue)
		return false;
	size_t nextpos = (aQueue->iFrontPos + QUEUE_MAXCOUNT - 1) % QUEUE_MAXCOUNT;
	if (nextpos == aQueue->iBackPos)
		return false;
	assert(queue_invariant(aQueue));
	aQueue->iFrontPos = nextpos;
...
bool queue_pop_back(struct TQueue* aQueue)
	...
	if (queue_is_empty(aQueue))
		return false;
	assert(queue_invariant(aQueue));
	aQueue->iBackPos = (aQueue->iBackPos - 1 + QUEUE_MAXCOUNT) % QUEUE_MAXCOUNT;
	assert(queue_invariant(aQueue));
	return true;
	}
