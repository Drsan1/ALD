...
bool queue_clone(const struct TQueue* aSourceQueue, struct TQueue* aCloneQueue)
	{
	if (!aSourceQueue || !aCloneQueue)
		return false;
	queue_destroy(aCloneQueue);
	queue_init(aCloneQueue);

	if (queue_is_empty(aSourceQueue))
		return true;
	struct TQueueNode* current = aSourceQueue->iFront;
	while (current) {
		if (!queue_push(aCloneQueue, current->iValue)) {
			queue_destroy(aCloneQueue);
			queue_init(aCloneQueue);
			return false;
		}
