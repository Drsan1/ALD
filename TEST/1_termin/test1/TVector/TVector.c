...
{
	if (!aSourceVector || !aVectorA || !aVectorB)
		return false;

	assert(vector_invariant(aSourceVector));

	size_t splitPos = 0;
	bool foundValue = false;

	for (size_t i = 0; i < aSourceVector->iSize; i++) {
		if (aSourceVector->iValues[i] == aValue) {
			splitPos = i;
			foundValue = true;
			break;
		}
	...
	if (!foundValue) {
		splitPos = aSourceVector->iSize;
	}

	size_t sizeA = splitPos;
	size_t sizeB = aSourceVector->iSize - splitPos - 1;

	if (!vector_init(aVectorA, sizeA)) {
		return false;
	}

	if (!vector_init(aVectorB, sizeB)) {
		vector_destroy(aVectorA);
		return false;
	}

	for (size_t i = 0; i < sizeA; i++) {
		aVectorA->iValues[i] = aSourceVector->iValues[i];
	}

	for (size_t i = 0; i < sizeB; i++) {
		aVectorB->iValues[i] = aSourceVector->iValues[splitPos + i + 1];
	}
